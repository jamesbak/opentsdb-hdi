...
echo "$(date +'%Y-%m-%d %H:%M:%S') Starting custom action script for provisioning OpenTSDB as an Ambari service"
apt-get install jq
...
echo "$(date +'%Y-%m-%d %H:%M:%S') Cluster: $cluster, Stack: $stack_name-$stack_version" 
...
echo "$(date +'%Y-%m-%d %H:%M:%S') Refreshing and restarting Ambari server"
...
if [ "$(hostname -i)" == "$head_ip" ] ; then
    ...
    echo "$(date +'%Y-%m-%d %H:%M:%S') Processing service registration on active head node via background script"
    ...
    echo "$(date +'%Y-%m-%d %H:%M:%S') Logging background activity to /var/log/opentsdb/create-ambari-services.out & /var/log/opentsdb/create-ambari-services.err"
    ...
    echo "$(date +'%Y-%m-%d %H:%M:%S') Node is not currently active head node"
fi
echo "$(date +'%Y-%m-%d %H:%M:%S') OpenTSDB has been installed and TSD components have been deployed to all HBase region servers"
